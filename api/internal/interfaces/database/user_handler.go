...
func (repository *UserServiceRepository) FindUser() (users []*entity.User, err error) {
	rows, err := repository.Query("SELECT id, first_name, last_name FROM users")
	if err != nil {
		return
	}
	defer rows.Close()
...
func (repository *UserServiceRepository) Save(u *entity.User) (id int, err error) {
...
func (repository *UserServiceRepository) Edit(identifier int, u *entity.User) (id int, err error) {
	return
}
